    ...
    public class BookingService : IBookingService
    {
        ...
        public async Task<List<BookingDetailStatusForBookingResponseDto>> GetBookingDetailStatusByBooking(int bookingId)
        {
            try
            {
                ...
                {
                    if (item.IsAccepted == false || item.IsNew == false)
                    {
                        list.Remove(item);
                    }
                }
        ...
        private static int GetMinEstimatedTime(int i, List<Booking> listBooking)
        {
            ...
            .Min(l => /*l.BookingDuration*/ l.BookingDuration);
        ...
        private static (int? bookingInFirstHourCount, int? bookingInNextHourCount) CountBookingPerHour(int num, int i, List<Booking> listBooking)
        {
            ...
            .Where(b => b.BookingTime.TimeOfDay.Hours.Equals(i) && /*b.BookingDuration*/ b.BookingDuration > 1).Count();
        ...
        private static void CheckIfGarageAvailablePerHour(int openAt, int closeAt, List<Booking> listBooking, int lotCount, List<BookingPerHour> listHours, DateTime dateSelect)
        {
            for (int i = openAt; i <= closeAt; i++)
            {
                ...
                /*b.BookingDuration*/ b.BookingDuration == 1).Count();
                        ...
                        for (int z = 1; z <= remainHour; z++)
                        {
                            ...
                            if (bookingInFirstHourCout + bookingInNextHourCout == lotCount && minEstimatedTimePerHour > 1)
                            {
                                var durationFirstHour = listBooking?.Where(b => b.BookingTime.TimeOfDay.Hours.Equals(i) &&
                                 /*b.BookingDuration*/ b.BookingDuration > 1).FirstOrDefault()!./*BookingDuration*/BookingDuration;
                                if (durationFirstHour > 1)
                                {
                                    ...
                                    i = i + z - 1;
                                    var isBookin = listBooking?.Where(b => b.BookingTime.TimeOfDay.Hours.Equals(i + 1) && /*b.BookingDuration*/ b.BookingDuration > 1).Count();
        ...
        private async Task RunManager(BookingCreateForManagerRequestDto requestDto, DateTime bookingTime, int totalEstimated)
        {
            try
            {
                ...
                booking.BookingDuration = totalEstimated;
                booking.BookingDuration = totalEstimated + 1;
        ...
        private async Task<PaymentLinkDto> Run(BookingCreateRequestDto requestDto, DateTime bookingTime, int totalEstimated)
        {
            try
            {
                ...
                var checkOut = await RunCheckOut(checkOutDto, true, bookingId, bookingTime);
        ...
        private async Task<(decimal, decimal, decimal)> RunCheckOut(CheckOutRequestDto requestDto, bool isCreateNew, int? bookingId, DateTime? bookingTime)
        {
            try
            {
                switch (false)
                ...
                for (int i = 0; i < listService.Count; i++)
                {
                    //get default product id
                    decimal productPrice = 0;
                    (decimal servicePrice, int? serviceWarranty) = serviceRepository.GetPriceAndWarranty(listService[i]);
                    ...
                    if (isCreateNew == true)
                    {
                        ...
                        var bookingDetail = new BookingDetail()
                        {
                            ProductPrice = productPrice,
                            ServicePrice = servicePrice,
                            ProductWarrantyEndDate = product is null ?
                            null : bookingTime!.Value.AddMonths((int)product!.ProductWarrantyPeriod!),
                            ServiceWarrantyEndDate = serviceWarranty is null ?
                            null : bookingTime!.Value.AddMonths((int)serviceWarranty!),
                            BookingServiceStatus = BookingServiceStatus.NotStart,
                            ProductId = product is null ? null : product.ProductId,
        ...
        public async Task UpdateBookingDetailForManager(int bookingDetailId, int productId)
        {
            try
            {
                var bookingDetailOld = await bookingDetailRepository.Detail(bookingDetailId);
                if (bookingDetailOld is not null)
                {
                    ...
                    var bookingDetailNew = mapper.Map<BookingDetail>(bookingDetailOld,
                    otp => otp.AfterMap((src, des) =>
                    {
                        des.BookingId = booking!.BookingId;
                        des.ProductId = productNew!.ProductId;
                        des.ProductPrice = productNew.ProductPrice;
                        des.ProductWarrantyEndDate =
                        booking!.BookingTime.AddMonths((int)productNew.ProductWarrantyPeriod!);
                    }));
            ...
            try
            {
                ...
                switch (bookingStatus)
                {
                    case BookingStatus.Canceled:
                        ...
                        if (TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow.AddHours(4), _timeZone) <=
                        booking.BookingTime)
                        {
                            await fCMSendNotificationMobile.SendMessagesToSpecificDevices
                            (booking.Car.Customer.User.DeviceToken, "Thông báo:",
                    ...
                    obj => obj.AfterMap((src, des) =>
                    {
                        for (int i = 0; i < src.Count; i++)
                        {
                            if (src[i].Product is not null)
                            {
                                ...
                                des[i].ServiceWarranty =
                                src[i].ServiceWarrantyEndDate is null ? string.Empty : src[i].ServiceWarrantyEndDate!.Value.ToString("dd/MM/yyyy");
                                des[i].ProductWarranty = src[i].ProductWarrantyEndDate!.Value.ToString("dd/MM/yyyy");
                            }
                            else
                            {
        ...
        public async Task<List<BookingListByCalenderResponseDto>> GetBookingByGarageCalendar(int? garageId)
        {
            try
            {
                var listObj = await bookingRepository.GetBookingByGarageCalendar(garageId);

                var listDto = mapper.Map<List<BookingListByCalenderResponseDto>>(listObj);
        ...
        public async Task ConfirmChangeInBookingDetail(int bookingId, bool isAccepted)
        {
            try
            {
                var bookingDetailList = new List<BookingDetail>();
                var list = await bookingDetailRepository.FilterAllBookingDetailByBooking(bookingId);
                var bookingDetail = list.Where(b => b.IsAccepted == false).FirstOrDefault();
                if (isAccepted == true)
                {
        ...
        public async Task CreateWarrantyForBooking(BookingCreateWarrantyRequestDto requestDto)
        {
            try
            {
                ...
                otp => otp.AfterMap((src, des) =>
                {
                    ...
                    for (int i = 0; i < bookingDetailList.Count; i++)
                    {
                        var serviceDuration = bookingDetailList[i]
                        .ServiceDetail.Service.ServiceDuration;
                        totalEstimated += serviceDuration;

                        if (bookingDetailList[i].ProductWarrantyEndDate < warrantyTime)
                        {
                            price += bookingDetailList[i].ProductPrice;
                        }
                    }
                    ...
                    des.BookingTime = warrantyTime;
                    //des.BookingDuration = totalEstimated;
                    des.BookingDuration = totalEstimated + 1;
                    des.WarrantyReason = requestDto.Reason;
                }));
        ...
        public async Task<GenericObject<List<BookingListResponseDto>>> ViewAndFilter(ViewAllAndFilterBooking page)
        {
            try
            {
                (var listObj, var count) = await bookingRepository.ViewAndFilter(page);
